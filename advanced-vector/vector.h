...
public:
    ...
    //¬ константном операторе [] используетс€ оператор  const_cast, 
    //чтобы сн€ть константность с ссылки на текущий объект и вызвать неконстантную версию оператора [].
    //“ак получитс€ избавитьс€ от дублировани€ проверки assert(index < size).
    const T& operator[](size_t index) const noexcept {
    ...
    // ћетод Reserve предназначен дл€ заблаговременного резервировани€ пам€ти под элементы вектора, когда известно их примерное количество
